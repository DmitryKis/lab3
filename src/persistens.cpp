...
int peresech(
        ...
        bool& fact)
{
    double slope1, slope2; // Eiyooeoeaiou oaeia iaeeiia o?aaiaiee i?yiuo, ia
                           // eioi?uo ?aniiei?aiu io?acee
    bool f1 = false, f2 = false;     // Eiae?aneea oeaae aey i?yiuo aeaa x = a
    double yIntercept1, yIntercept2; // Naiaiaiua ?eaiu o?aaiaiee i?yiuo, ia
                                     // eioi?uo ?aniiei?aiu io?acee
    double xIntersection,
            yIntersection; // Eii?aeiaou oi?ee ia?ana?aiey, anee iia nouanoaoao
    ...
    slope2
            = (vx2 != 0
                       ? getSlope(x3, y3, x4, y4)
                       : 0); // I?enaaeaaai cia?aiea 0 eiyooeoeaioo oaea iaeeiia
    vx1 != 0 ? yIntercept1 = getYIntercept(x1, y1, x2, y2)
             : f1 = true; // Aey i?yiuo aeaa x = a
    vx2 != 0 ? yIntercept2 = getYIntercept(x3, y3, x4, y4)
             : f2 = true; // I?enaaeaaai oeaao cia?aiea true
    if (!f1 && !f2) {
    ...
    } // Aeie iaoi?aaiey ?aniiei?aiey aey anao io?aceia, e?iia oao, aaa oioy au
      // iaei ec ieo ?aniiei?ai ia i?yiie aeaa x = a
    else {
        if (slope1 == 0 && slope2 == 0 && f1 && f2) {
            ...
            if (getCos(x1 - xIntersection,
                ...
                /*cout << "Don't intersect";*/ fact = false;
        } // Aeie iaoi?aaiey ?aniiei?aiey, eiaaa oieuei iaia i?yia eiaao aea x =
          // a, a aoi?ay y = kx +b
    } // Aeie iaoi?aaiey ?aniiei?aiey, anee oioy au iaei io?acie iaoiaeony ia
      // i?yiie aeaa x = a
    return 0;
}
